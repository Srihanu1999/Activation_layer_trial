...
__global__ void relu_backward_kernel(float* input, float* grad_output, int size, float* grad_input) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    idx = min(idx, size - 1);  // Prevent out-of-bounds access, no if statement
...
__global__ void leaky_relu_backward_kernel(float* input, float* grad_output, int size, float alpha, float* grad_input) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    idx = min(idx, size - 1);  // Prevent out-of-bounds access, no if statement
    grad_input[idx] = grad_output[idx] * (1.0f * (input[idx] > 0.0f) + alpha * (1.0f - (input[idx] > 0.0f)));  // Grad for Leaky ReLU
}

// Kernel for backward pass of Sigmoid (completely no if statements)
__global__ void sigmoid_backward_kernel(float* input, float* grad_output, int size, float* grad_input) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    idx = min(idx, size - 1);  // Prevent out-of-bounds access, no if statement
...
__global__ void tanh_backward_kernel(float* input, float* grad_output, int size, float* grad_input) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    idx = min(idx, size - 1);  // Prevent out-of-bounds access, no if statement
